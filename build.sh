...
sysctl -p # reload /etc/sysctl.conf rules
...
-A PREROUTING -s 172.16.0.0/24 -i eth0 -p udp -m udp --dport 53 -j REDIRECT --to-ports 9053 -m comment --comment "Redirect DNS to DNSPort"
-A PREROUTING -s 172.16.0.0/24 -i eth0 -p tcp -m tcp --dport 53 -j REDIRECT --to-ports 9053 -m comment --comment "Redirect DNS to DNSPort"
-A PREROUTING -s 172.16.0.0/24 -i eth0 -p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK SYN -j REDIRECT --to-ports 9040 -m comment --comment "Redirect remaining TCP to TransPort"
COMMIT
...
-A INPUT -m state --state INVALID -j DROP -m comment --comment "Drop invalid TCP packets"
-A INPUT -p tcp --tcp-flags ALL ACK,RST,SYN,FIN -j DROP -m comment --comment "Drop invalid SYN flags"
-A INPUT -p tcp --tcp-flags SYN,FIN SYN,FIN -j DROP -m comment --comment "Drop invalid SYN flags"
-A INPUT -p tcp --tcp-flags SYN,RST SYN,RST -j DROP -m comment --comment "Drop invalid SYN flags"
-A INPUT -p tcp --tcp-flags ALL ALL -j DROP -m comment --comment "Drop xmas SYN"
-A INPUT -p tcp --tcp-flags ALL NONE -j DROP -m comment --comment "Drop null SYN"
-A INPUT -f -j DROP -m comment --comment "Drop incoming fragmented packets"
...
-A FORWARD -j REJECT --reject-with icmp-admin-prohibited -m comment --comment "No forwards"
COMMIT
